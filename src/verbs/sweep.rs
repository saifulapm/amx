...
//! is the forge saying the request is over, or git saying every commit on the
//! branch is in the main line: both are facts somebody else established, and
//! neither can be established by a wall going quiet.
//!
...
/// Why this agent is on the list, or nothing at all.
///
/// A turn that is over or an agent sitting idle, on a branch of amx's cutting,
/// whose work has landed. The forge is asked first and only through what the
...
fn reason(view: &View) -> Option<String> {
    ...
    settled(&pr::written(&view.meta)).or_else(|| in_the_main_line(&view.meta, branch))
}
