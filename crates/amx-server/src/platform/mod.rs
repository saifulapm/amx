...
//! the process table (`/proc` on Linux, libproc on macOS), watching a file for
//! changes (inotify on Linux, kqueue on macOS) — is in here so that a future
//! port implements the same traits against the same conformance tests and
