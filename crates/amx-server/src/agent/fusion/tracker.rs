...
use super::edge::{EdgeEffect, HookEdge, PERMISSION_PROMPT, precedence};
use super::{
    CONFIRMATION_CAP, CONFIRMATIONS, Deadline, Effect, IDENTITY_GRACE, Input, STALENESS,
    ScreenVerdict,
};
...
impl Tracker {
    ...
    fn on_hook(&mut self, edge: &HookEdge, effects: &mut Vec<Effect>) {
        if edge.subagent {
            return;
        }
    ...
    fn on_screen(&mut self, verdict: &ScreenVerdict, effects: &mut Vec<Effect>) {
    ...
    fn on_identified(&mut self, kind: AgentKind, effects: &mut Vec<Effect>) {
    ...
    fn on_probe(&mut self, activity: Activity, effects: &mut Vec<Effect>) {
    ...
    /// A deadline fired.
    fn on_deadline(&mut self, deadline: Deadline, effects: &mut Vec<Effect>) {
    ...
    fn on_exited(&mut self, effects: &mut Vec<Effect>) {
        self.exited = true;
        let from = self.state;
        self.pending = None;
    ...
    fn enter(&mut self, state: AgentState, cause: StatusCause, effects: &mut Vec<Effect>) {
        if state == self.state {
    ...
    fn leave(&mut self, authority: ExitAuthority, effects: &mut Vec<Effect>) {
        if !self.state.is_held() {
            return;
        }
    ...
    /// timers.
    fn transition(&mut self, to: AgentState, cause: StatusCause, effects: &mut Vec<Effect>) {
        let from = self.state;
        self.state = to;
        self.cause = cause;
        effects.push(Effect::Status {
    ...
    fn clear_pending(&mut self, effects: &mut Vec<Effect>) {
    ...
    fn arm(&mut self, deadline: Deadline, after: Duration, effects: &mut Vec<Effect>) {
        self.armed.set(deadline, true);
        effects.push(Effect::Arm { deadline, after });
    }

    /// Release a timer, if one is held.
    fn disarm(&mut self, deadline: Deadline, effects: &mut Vec<Effect>) {
        if self.armed.set(deadline, false) {
