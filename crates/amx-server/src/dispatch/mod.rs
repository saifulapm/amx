...
//! `agent.report`, **V13** `agent.start`/`agent.prompt`. V12's four —
//! `pane.send_text`/`send_keys`/`run`/`read` — and V11's three —
//! `wait`/`pane.wait_output`/`events.subscribe` — are filled, and each left the
//! list with the count in `tests/hygiene.rs`. V17 empties it again and deletes
//! the helper, which is the milestone's exit check.
